...
class _OrderScreenState extends State<OrderScreen> with SingleTickerProviderStateMixin {
  ...
  double sum(){
    ...
    setState(() {
      for(int i=0;i<widget.cartItem.length;i++) {
        total += (widget.cartItem[i].price * widget.cartItem[i].quantity)+widget.cartItem[i].optionPrice;
      }
    });
    return total;
  }
  ...
  Widget _buildFoodItem() {
    ...
    return Container(
      ...
      ): widget.cartItem.length==0?new Container(
          decoration: BoxDecoration(
                              ...
                              onTap: () {
                                Navigator.of(context).push(MaterialPageRoute(
                                    builder: (context) => DetailsPage(
                                        ...
                                        foodName: widget.cartItem[index].name,
                                        foodPrice: widget.cartItem[index].price,
                                        order: true,
                                        refresh: refresh)));
                              },
                              child: Row(
                                ...
                                children: <Widget>[
                                            ...
                                            setState(() {
                                              Navigator.of(context).push(
                                                  MaterialPageRoute(
                                                      builder: (context) =>
                                                          DetailsPage(
                                                              heroTag: "assets/images/salmon.png", foodName: widget.cartItem[index].name, foodPrice: widget.cartItem[index].price, edit: true, index: index, order: true, refresh: refresh)));
                                            });
                                          },
                                        ),
                                      ]),
                                  Container(
                                    ...
                                    Column(
                                        ...
                                        children: [
                                          Text(widget.cartItem[index].name,
                                              style: TextStyle(
                                                  fontFamily: 'Montserrat',
                                                  fontSize: 17.0,
                                                  fontWeight: FontWeight.bold)),
                                          ...
                                          Text(
                                              (widget.cartItem[index].price *
                                                          widget.cartItem[index]
                                                              .quantity+widget.cartItem[index].optionPrice)
                                                      .toString() +
                                                  " " +
                                                  "L.e",
                                              style: TextStyle(
                                                  ...
                                                  fontSize: 15.0,
                                                  color: Colors.grey)),
                                          widget.cartItem[index].optionName==''?new Container():Text(
                ...
                children: <Widget>[
                  ...
                  GestureDetector(
                    onTap: () async {
                      ...
                      dynamic result = await DatabaseService(uid: user.uid).addOrder(
                          widget.cartItem);
                      print(result);
                      if (result == null) {
                        setState(() {
